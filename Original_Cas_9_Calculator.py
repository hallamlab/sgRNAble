...
def identifyTargetSequencesMatchingPAM(PAM_seq, positions_at_mers, full_sequence,
    ...
    list_of_mers_with_PAM = [
        mer + PAM_seq for mer in mers(mer_length - len(PAM_seq))]
        ...
        for Guide in self.guideSequences:
            ...
            for (source, targets) in targetDictionary.items():
                ...
                for fullPAM in self.Cas9Calculator.returnAllPAMs():
                    ...
                    dG_supercoiling = self.Cas9Calculator.calc_dG_supercoiling(
                    ...
                    for (target_sequence, targetPosition) in targetDictionary[source][fullPAM]:
                        dG_exchange = self.Cas9Calculator.calc_dG_exchange(
                        ...
                        self.targetSequenceEnergetics[source][targetPosition] = {'sequence': target_sequence,
                                                                                 ...
                                                                                 'dG_target': dG_target}
                        self.partition_function += math.exp(-dG_target /
                                                            self.Cas9Calculator.RT)
            ...
            for (source, targets) in list(self.targetSequenceEnergetics.items()):
                ...
                sortedTargetList = sorted(
                    list(targets.items()), key=lambda k_v: k_v[1]['dG_target'])
                print("POSITION\t\tTarget Sequence\t\tdG_Target\t\t% Partition Function")
                j = 0
                for (position, info) in sortedTargetList[0:numTargetsReturned]:
                    percentPartitionFunction = 100 * \
                        math.exp(-info['dG_target'] /
                                 self.Cas9Calculator.RT) / self.partition_function
                    print("%s\t\t\t%s\t\t\t%s\t\t\t%s" % (str(position), info['sequence'], str(
                        round(info['dG_target'], 2)), str(percentPartitionFunction)))
                    worksheet.write(row, col, str(position))
                    worksheet.write(row, col + 1, info['sequence'])
                    worksheet.write(
                        row, col + 2, str(round(info['dG_target'], 2)))
                    worksheet.write(
                        row, col + 3, str(percentPartitionFunction))
                    row = row + 1
...
class clCas9Calculator(object):
    ...
    def __init__(self, filename_list, quickmode=True, ModelName='InvitroModel.mat'):
        ...
        self.PAM_energy = {'GGA': -9.8, 'GGT': -10, 'GGC': -10, 'GGG': -9.9, 'CGG': -8.1, 'TGG': -7.8, 'AGG': -8.1, 'AGC': -8.1, 'AGT': -8.1, 'AGA': -7.9, 'GCT': -7.1, 'GCG': -6.9, 'ATT': -
                           7.2, 'ATC': -6.4, 'TTT': -7.6, 'TTG': -6.8, 'GTA': -7.4, 'GTT': -7.9, 'GTG': -7.7, 'AAT': -7, 'AAG': -7, 'TAT': -7.2, 'TAG': -7.2, 'GAA': -7.2, 'GAT': -7.3, 'GAC': -7.2, 'GAG': -7.3}

        self.initTargetFinder(filename_list)

    def returnAllPAMs(self):
        # PAMpart will be 'GGT'
        for (PAMpart, energies) in sorted(list(self.PAM_energy.items()), key=lambda x: x[1]):
    ...
    def initTargetFinder(self, filename_list):
        ...
        for filename in filename_list:
            ...
            positionsAtMers = identifyNucleotidePositionsOfMers(
                fullSequence, length=10)
            targetDictionary[filename] = {}
            targetSequenceList = []
            for fullPAM in self.returnAllPAMs():
                targetSequenceList = identifyTargetSequencesMatchingPAM(
                    fullPAM, positionsAtMers, fullSequence)
