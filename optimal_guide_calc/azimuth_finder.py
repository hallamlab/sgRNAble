...
def PAM_Finder(Sequence, PAM, Direction, Cutoff):
    ...
    Guide_RNAs = [x for y, x in sorted(
        zip(predictions, Guide_RNAs), reverse=True)][:Azimuth_Cutoff]
    Location = [x for y, x in sorted(
        zip(predictions, Location), reverse=True)][:Azimuth_Cutoff]
    Strand = [x for y, x in sorted(
        zip(predictions, Strand), reverse=True)][:Azimuth_Cutoff]
...
def Azimuth_Guides(Gene_Seq, PAM_Seq, Guides_Cutoff, Usage):
    ...
    if (Usage == "CRISPRi Screening"):
                ...
                for i in range(len(Contig_Frame)):
                    ...
                    if Contig_Frame[i*3:(i*3)+3].seq in Stop_Codons:
                        ...
                        if(i*3-Location_in_Contig) > ORF_Cutoff and Location_in_Contig != 0:
                            Gene1, Location1, Direction1, Azimuth_Score1 = PAM_Finder(
                                Contig_Frame[Location_in_Contig:(i*3)+3].seq, PAM_Seq, 1, Guides_Cutoff)
                            Position.append(Location1)
                            Direction.append(Direction1)
                            Azimuth_Rank.append(Azimuth_Score1)
                            Meth_Distances = []
                            for Loc in Location1:
                                Meth_Distances.append(ClosestDiff(
                                    Start_Codons, Loc+Location_in_Contig))
                            Distance_to_Nearest_Met.append(Meth_Distances)
                            Guides.append(Gene1)
                            Contig_ORFs.append(
                                Contig_Frame[Location_in_Contig:(i*3)+3].seq)
        ...
        for Gene in Gene_Seq:
            Gene, Location, Direction, Azimuth_Score = PAM_Finder(
                Gene.seq.upper(), PAM_Seq, -1, Guides_Cutoff)
        ...
        Gene1, Location1, Direction1, Azimuth_Score1 = PAM_Finder(
            Gene_Seq, PAM_Seq, 1, Guides_Cutoff)
        Gene2, Location2, Direction2, Azimuth_Score2 = PAM_Finder(
            Gene_Seq, PAM_Seq, -1, Guides_Cutoff)
